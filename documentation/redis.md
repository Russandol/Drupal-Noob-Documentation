...
services:
  # Backend pour le checksum des caches tags. Utilisé par Redis et la plupart des autres backends
  # pour gérer l'invalidation des caches tags.
...
> ⚠️ Il faut bien importer ces fichiers de configuration AVANT d'importer le fichier *settings.local.php* pour éviter les conflits. Par
> exemple : si on souhaite désactiver le cache dans le fichier *settings.local.php*, celà sera ensuite surchargé par la configuration
> du fichier *redis.php*.
