export default {
  text: 'Installation de Drupal',
  collapsed: true,
  items: [
    { text: 'Introduction', link: '/drupal-installation/' },
    { text: 'DDEV', link: '/drupal-installation/ddev' },
    // { text: 'Composer', link: '/drupal-installation/composer' },
    // { text: 'Premier module : Drush', link: '/drupal-installation/drush' },
    // { text: 'Installation de Drupal', link: '/drupal-installation/installation'}


    // { text: 'Modifier le host', link: '/02-drupal-installation/host' },
    // { text: 'Commandes make pour Drupal', link: '/02-drupal-installation/drupal-makefile' },
    // { text: 'Architecture d\'un projet Drupal', link: '/02-drupal-installation/architecture' },
    // { text: 'Premier module : Drush', link: '/02-drupal-installation/drush' },
    // { text: 'Fichiers settings', link: '/02-drupal-installation/settings-files' },
    // { text: 'Mise à jour .gitignore', link: '/02-drupal-installation/maj-gitignore' },
    // { text: 'Le cache avec Redis', link: '/02-drupal-installation/redis' },
    // { text: 'les fichiers de configuration', link: '/02-drupal-installation/config' },
    // { text: 'Mise à jour Readme', link: '/02-drupal-installation/maj-readme' },
  ]
}