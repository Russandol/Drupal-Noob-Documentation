...
- Lance une session `bash` interactive dans ce conteneur
...
Cette commande (corrigée, car la syntaxe originale contient une erreur) :
- Exécute l'action pour arrêter et supprimer tous les conteneurs, réseaux et volumes créés par `up` `down`
